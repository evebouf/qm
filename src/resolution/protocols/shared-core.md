...
## Using skills
Read `skills/<name>/SKILL.md` first: core serves instructions without a sandbox. Declare assets with `skills: ["name"]` on each execute/start_process (sandbox exec/start_process); reads and command paths install nothing. Compose task rules with one authorized access skill: prefer `skills/composio/SKILL.md` when its credential is available, otherwise the direct connector. Preserve allowlists and approvals. Never switch credentials to evade denial.
