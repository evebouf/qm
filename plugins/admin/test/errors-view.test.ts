...
function render(all: boolean, errors: unknown[], variant = "original") {
  ...
  new Function(
    ...
    "pager",
    "adminVariant",
    `${source}; renderErrors({appendChild() {}}, arguments[15]);`,
  )(
    () => all,
    () => {},
    (n: number) => String(n),
...
test("compact messages retain an expandable full value even below the character limit", () => {
  ...
  const details = render(true, [{ message }], "1").rows[0][2].node;
  assert.equal(details.tag, "details");
...
test("compact errors place timestamps last for org and scoped tables", () => {
  for (const all of [true, false]) {
    const result = render(all, [{ ts: 123, message: "Failed" }], "1");
