...
for (const size of [199, 200, 201]) {
  test(`thread pagination includes parent in the ${size}-message boundary`, async () => {
    ...
    await cache.ingest(Array.from({ length: size }, (_, i) => ({
      container: "C1", ts: ts(i), text: `message ${i}`, ...(i ? { sub: ts(0) } : {}),
    })));
    ...
    assert.deepEqual(page.raw.map((m) => m.ts), Array.from({ length: Math.min(size, 200) }, (_, i) => ts(i)));
...
test("before excludes future replies and a parent outside the requested window", async () => {
  const cache = createMemorySurfaceCache();
  await cache.ingest(Array.from({ length: 205 }, (_, i) => ({
    container: "C1", ts: ts(i), text: "message", ...(i ? { sub: ts(0) } : {}),
  })));
  const core = { readSurfaceMessages: cache.readMessages } as unknown as SlackCoreClient;
...
test("missing parent does not consume a reply slot when a bounded read permits mirror use", async () => {
  const cache = createMemorySurfaceCache();
  await cache.ingest(Array.from({ length: 201 }, (_, i) => ({ container: "C1", ts: ts(i + 1), sub: ts(0), text: "reply" })));
  const core = { readSurfaceMessages: cache.readMessages } as unknown as SlackCoreClient;
...
test("channel overflow keeps newest roots, excludes deletions and preserves visible system entries", async () => {
  const cache = createMemorySurfaceCache();
  await cache.ingest(Array.from({ length: 202 }, (_, i) => ({ container: "C1", ts: ts(i), text: "root", ...(i === 2 ? { subtype: "channel_topic" } : {}) })));
  ...
  assert.equal(page.hasMore, true);
  assert.deepEqual(page.raw.map((m) => m.ts), Array.from({ length: 200 }, (_, i) => ts(i + 1)));
